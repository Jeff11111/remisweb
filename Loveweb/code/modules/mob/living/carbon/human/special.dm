...
/*			if("succubus")
			...
			if("hitman")
				if(src.job == "Grayhound")
					...
					if(src.head)
						qdel(src.head)
					src.equip_to_slot_or_del(new /obj/item/clothing/under/hitman(src), slot_w_uniform)
					src.equip_to_slot_or_del(new /obj/item/clothing/shoes/lw/jackboots(src), slot_shoes)
					src.equip_to_slot_or_del(new /obj/item/clothing/glasses/badass_sunglasses(src), slot_glasses)
					src.my_skills.ADD_SKILL(SKILL_MELEE, 4)
					src.my_skills.ADD_SKILL(SKILL_RANGE, 5)
					src.my_stats.change_stat(STAT_ST , 3)
					src.my_stats.change_stat(STAT_HT , 2)
