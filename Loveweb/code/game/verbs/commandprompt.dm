		...
		switch(chosenoption)
			...
			if("togglefuta")
				if(src.mob.type != /mob/new_player)
					to_chat(usr, "<span class='combat'>You're not in the lobby!</span>")
					return
				var/futa_message = "<span class='highlighttext'>You're hiding a big secret. (⌐ ͡■ ͜ ͡■)</span>" // ;}
				if(src.prefs.togglefuta)
					futa_message = "<span class='highlighttext'>You're not hiding anything.</span>"
				to_chat(usr, "[futa_message]")
				src.prefs.togglefuta = !src.prefs.togglefuta
			...
			if("togglefemboy")
				if(src.mob.type != /mob/new_player)
					to_chat(usr, "<span class='combat'>You're not in the lobby!</span>")
					return
				var/femboy_message = "<span class='highlighttext'>You have a certain personality.</span>"
				if(src.prefs.togglefemboy)
					femboy_message = "<span class='highlighttext'>You are now what your parents wants you to be.</span>"
				to_chat(usr, "[femboy_message]")
				src.prefs.togglefemboy = !src.prefs.togglefemboy
				prefs.save_preferences()
				prefs.savefile_update()
				return
			if("togglesize")
